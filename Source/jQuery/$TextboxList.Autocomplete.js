//lines 61 && 65: Usage of fromTrigger, which is used to control the display of the autocomplete prompt depending on whether or not the show() function was triggered by a real event
//lines 84 & 89 : Added null checks on result values array before values from it are attempted to be shown (showResults())
//line 93: Added check on existence of method obj and its member function named "filter" to make filter method application more robust (and make filtering optional as a result)
//lines 232-240: Added method obj with no filter member function in order to prevent client-side (additional) filtering
...
$.TextboxList.Autocomplete = function(textboxlist, _options){
  ...
  var index, prefix, method, container, list, values = [], searchValues = [], results = [], placeholder = false, current, currentInput, hidetimer, doAdd, currentSearch, currentRequest;
			...
			if (searchValues[search]){
				...
				currentRequest = $.ajax({
					...
					success: function(r){
						...
						values = r;
						if(values !== null) showResults(search);
					}
				});
			}
		}
	...
	var showResults = function(search){
		var results = (method && method.hasOwnProperty("filter") ? method.filter(values, search, options.insensitive, options.maxResults) : values);
